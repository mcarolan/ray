  ...
  spec = do
    ...
    describe "Quad" $ do
      it "identifies the x component" $ do
        x (Quad 4.3 (-4.2) 3.1 1.0) `shouldApproxBe` 4.3

      it "identifies the y component" $ do
        y (Quad 4.3 (-4.2) 3.1 1.0) `shouldApproxBe` (-4.2)

      it "identifies the z component" $ do
        z (Quad 4.3 (-4.2) 3.1 1.0) `shouldApproxBe` 3.1

      it "identifies the w component" $ do
        w (Quad 4.3 (-4.2) 3.1 1.0) `shouldApproxBe` 1.0
      ...
      it "allows points to be constructed" $ do
        (point 1.0 2.0 3.0) `shouldApproxBe` (Quad 1.0 2.0 3.0 1.0)

      it "allows vectors to be constructed" $ do
        (vector 1.0 2.0 3.0) `shouldApproxBe` (Quad 1.0 2.0 3.0 0.0)
      ...
      describe "magnitude" $ do
        it "computes the magnitude of vector(1, 0, 0)" $ do
          magnitude (vector 1 0 0) `shouldApproxBe` 1
        it "computes the magnitude of vector(0, 1, 0)" $ do
          magnitude (vector 0 1 0) `shouldApproxBe` 1
        it "computes the magnitude of vector(0, 0, 1)" $ do
          magnitude (vector 0 0 1) `shouldApproxBe` 1
        it "computes the magnitude of vector(1, 2, 3)" $ do
          magnitude (vector 1 2 3) `shouldApproxBe` (sqrt 14)
        it "computes the magnitude of vector(-1, -2, -3)" $ do
          magnitude (vector (-1) (-2) (-3)) `shouldApproxBe` (sqrt 14)
