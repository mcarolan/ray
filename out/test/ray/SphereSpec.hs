...
spec = do
  it "computes the normal on a sphere at a point on the X axis" $ do
    normalAt sphere (point 1 0 0) `shouldApproxBe` vector 1 0 0
  it "computes the normal on a sphere at a point on the Y axis" $ do
    normalAt sphere (point 0 1 0) `shouldApproxBe` vector 0 1 0
  it "computes the normal on a sphere at a point on the Z axis" $ do
    normalAt sphere (point 0 0 1) `shouldApproxBe` vector 0 0 1
  it "computes the normal on a sphere at a point at a nonaxial point" $ do
    let x = sqrt 3 / 3
    normalAt sphere (point x x x) `shouldApproxBe` vector x x x
  it "computes the normal on a sphere at a point at a nonaxial point" $ do
    let x = sqrt 3 / 3
